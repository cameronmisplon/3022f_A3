...
namespace MSPCAM001
{
	...
	PGMIMAGEPROCESSOR::PGMimageProcessor(const PGMIMAGEPROCESSOR & rhs): pixels(nullptr), Ncols(rhs.Ncols), Nrows(rhs.Nrows), components(), name(rhs.name)
	{
		...
		{
			(this->components).push_back( std::unique_ptr<MSPCAM001::ConnectedComponent>(new MSPCAM001::ConnectedComponent(*(rhs.components[i]))));
		}
	}
	...
	void PGMIMAGEPROCESSOR::readImage(void)
	{
		...
		getline(in,comments);
		getline(in,comments);
		while (comment.substr(0,1).equals("#")){
			getline(in,comments);
		}
	...
	int PGMIMAGEPROCESSOR::extractComponents(unsigned char threshold, int minValidSize)
	{
		...
		for (int i=0; i<Ncols*Nrows; ++i)
		{
			...
			pixels[i]=0;
			if (pixel >= static_cast<float>(threshold)){
				std::cout << pixel << " : starter" << std::endl;    //1
	...
	bool PGMIMAGEPROCESSOR::writeComponents(const std::string & outFileName)
	{
		...
		for (MSPCAM001::ConnectedComponent component : components)
		{
